<div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-10">
        ...
        <div class="flex space-x-6 overflow-x-auto pb-4 scrollbar-hide">
            <!-- Milk -->
            <a href="{{ route('catalog', ['category' => 'milk']) }}"
                class="flex flex-col items-center group min-w-[80px]">
                <div class="w-20 h-20 rounded-2xl flex items-center justify-center mb-2 transition-colors bg-gray-100 group-hover:bg-purple-50">
            ...
            <!-- Water -->
            <a href="{{ route('catalog', ['category' => 'water']) }}"
                class="flex flex-col items-center group min-w-[80px]">
                <div class="w-20 h-20 rounded-2xl flex items-center justify-center mb-2 transition-colors bg-gray-100 group-hover:bg-purple-50">
                    <span class="text-3xl">💧</span>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <!-- Banner 1 -->
            <div class="bg-[#3B82F6] rounded-xl p-6 text-white relative overflow-hidden h-40 flex flex-col justify-center">
                ...
                <button class="bg-white/20 hover:bg-white/30 text-white text-xs font-bold px-4 py-1.5 rounded-lg w-fit transition relative z-10 backdrop-blur-sm">Order Now</button>
            </div>
            <!-- Banner 2 -->
            <div class="bg-[#06B6D4] rounded-xl p-6 text-white relative overflow-hidden h-40 flex flex-col justify-center">
            ...
            <!-- Banner 3 -->
            <div class="bg-[#F97316] rounded-xl p-6 text-white relative overflow-hidden h-40 flex flex-col justify-center">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-12">
        ...
        <div id="category-{{ strtolower($category) }}">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="#" class="text-purple-600 font-bold text-xs uppercase tracking-wide hover:text-purple-700">see all</a>
            </div>

            <!-- Horizontal Scroll Container -->
            <div class="flex overflow-x-auto pb-8 -mx-4 px-4 space-x-4 scrollbar-hide">
                @foreach($products as $product)
                <div class="min-w-[200px] w-[200px] bg-white dark:bg-gray-800 rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow duration-200 border border-gray-50 dark:border-gray-700 relative group">
                    <!-- Discount Badge -->
                    <div class="absolute top-0 left-0 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded-br-lg rounded-tl-xl z-10 shadow-sm">
                        13% OFF
                    </div>
                    ...
                    <div x-data="{
                        ...
                        <!-- Image Area -->
                        <div class="h-32 w-full flex items-center justify-center mb-3 relative rounded-lg bg-gray-50 dark:bg-gray-700/50 overflow-hidden">
                            <template x-if="image">
                                <img :src="image" class="h-full w-full object-cover transition duration-300 transform group-hover:scale-105" alt="{{ $product->name }}">
                            </template>
                            <template x-if="!image">
                        ...
                        <div class="flex flex-col flex-grow">
                            <a href="{{ route('product.show', $product) }}" class="block group-hover:text-purple-600 transition-colors">
                                <h3 class="text-sm font-bold text-gray-900 dark:text-gray-100 leading-tight mb-1 line-clamp-2 h-10">{{ $product->name }}</h3>
                            </a>
                            ...
                            <div class="mt-auto pt-2 border-t border-gray-100 dark:border-gray-700/50">
                                <div class="flex items-center justify-between">
                                    <div class="flex flex-col">
                                        <span class="text-xs text-gray-400 line-through font-medium">₹<span x-text="Math.round(price * 1.2)"></span></span>
                                        <span class="text-base font-bold text-gray-900 dark:text-white">₹<span x-text="price"></span></span>
                                    </div>
                                ...
                                <div class="flex items-center space-x-1 mt-3 overflow-x-auto scrollbar-hide">
                                    <template x-for="variant in variants" :key="variant.id">
                                        <button @click="selectedVariant = variant.id; price = variant.price; unit = variant.unit; if(variant.image) image = variant.image;"
